...
try {
  ...
  const fileLevelTypes = new Set(['file', 'config', 'document', 'service', 'pipeline', 'table', 'schema', 'resource', 'endpoint', 'data', 'markup']);
  ...
  graph.layers.forEach(layer => {
    ...
    if (!layer.description) issues.push(`Layer '${layer.id}' missing description`);
    if (!layer.nodeIds) issues.push(`Layer '${layer.id}' missing nodeIds`);
