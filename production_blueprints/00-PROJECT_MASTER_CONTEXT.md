...
| **`09-TEAM_WORKFLOW.md`** | The two-repo team's actual working model — contract-first, mocked frontend, connect later, given the team works from home and can't reach GitLab until at the office. Identical copy in both `cpf-backend` and `cpf-frontend`. | You're figuring out how backend and frontend coordinate without stepping on each other. |
| **`10-API_CONTRACT_V0.md`** | A hand-written, pre-Step-3 API contract — exact endpoint shapes, JSON casing (camelCase), the `?lang=` convention, flattened-translation response shape — so Step 3 (backend) and Step 4 (frontend) can start in parallel without guessing the same undecided details differently. Superseded by the real OpenAPI export once Step 3 actually ships. | You're about to write a Ninja schema or a frontend fixture and need the exact field names to target. |
...
6. **"Join Our Team" and "Submit Your Story" field lists diverge from their existing (previously unmodeled) mockup implementations** — `Contact.jsx`'s careers modal and `success/ShareStoryModal.jsx` each collect a different field set than what's now specified (see `07-...md` §4.4 for the exact diff). Schema currently implements the newly given field lists exactly; whether to reconcile fields back in (specialization/job-type on the team form; title/photo/video on the story form) is a content/product decision, not yet made. `01-...md` §10.2, §10.3.
7. **`CPF TIMELINE.xlsx` still hasn't actually been delivered**, despite `01-...md` §5 originally recording it as "available now, no blocker" — corrected once Step 2 implementation went looking for the file and it wasn't there. The mockup's own smaller 4-entry timeline (2015/2017/2019/2024) was seeded as an interim placeholder; real ingestion is still blocked on the actual spreadsheet. `01-...md` §5.
8. **The About page quote is a placeholder**, not a real selection from `HRHCP's Quotes - DK 11.2 - Copy.xlsx` — that spreadsheet has never actually been provided. Needs explicit client sign-off before launch, per `01-...md` §3.8's own instruction (this was already correctly flagged as pending; recorded here again because Step 2 hit it directly).

*(Resolved, no longer open: Foundation Leaders' "no mockup precedent" claim — corrected, see `01-...md` §10.1 and `07-...md` §4.2. `AboutPage.jsx`'s `leaders{board, executive}` array was found once implementation started; both the data shape and the page placement are now settled by that precedent, not open questions.)*
