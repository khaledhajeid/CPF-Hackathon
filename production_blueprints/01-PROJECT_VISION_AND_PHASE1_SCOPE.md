...
## 10. New scope additions (direct instruction, post-dating §2's source PDF and §9's IT notes)
...
A team/careers application form is now explicitly in scope with a defined field list (name, email, phone, resume, message). **This already exists as a partially-built feature in the mockup** — `Contact.jsx`'s "careers" tab has a working CV-submission modal — but that mockup form collects a different field set (name, a required specialization/interest-area field, and a required CV upload; no email, phone, or message field) and distinguishes two submission types via `modalType` ("بوابة الوظائف والخبرات" / jobs vs. "بوابة تدريب الشباب" / training) that the new field list doesn't capture. **This needs a content/product decision, not an engineering guess:** does the new field list supersede the mock's design intentionally (drop `interest_area` and the job/training distinction), or should those be reconciled back in? See `07-DATA_MODEL_ERD_RATIONALE.md` §4.4 for the full side-by-side comparison. The schema (`TeamJoinSubmission`) currently implements the newly given field list exactly, pending that decision.

### 10.3 "Submit Your Story" form — new, but a mockup precedent already exists and needs reconciling

A public-facing form for visitors to submit their own success story for editorial review is now explicitly in scope (name, email, phone, program name, story text). **This already exists as a fully-built feature in the mockup** — `success/ShareStoryModal.jsx`, a complete 3-step form — but, again, with a different field set: the mock collects an optional story title and an optional photo-or-video attachment (its own copy argues photo/video submissions "get much more engagement"), and collects no email or phone number at all. Same open question as §10.2: does the given field list intentionally drop title/photo/video capture, or should the schema (and the eventual frontend) restore them? See `07-DATA_MODEL_ERD_RATIONALE.md` §4.4 for the full comparison. The schema (`SuccessStorySubmission`) currently implements the newly given field list exactly, pending that decision.
