...
| 🟣 **NEW REQUIREMENT (DIRECT INSTRUCTION)** | Not in the mock, not in the original client PDF notes, not in the later IT alignment notes — added because it was directly instructed in a later session, sometimes with an exact structure given. Distinct from 🔵 because it isn't traceable to `01-...md`'s existing scope record at all — see `01-...md`'s own addendum sections for where this gets folded back into the scope-of-record. |
...
**`Leader` + `LeaderTranslation`** — 🟣 **entirely a new requirement, added on direct instruction.** Unlike every other entity in this section, "قيادات المؤسسة" (Foundation Leaders — board and executive leadership) has no precedent anywhere in this document set: it isn't in the mock, it isn't in the original client PDF notes translated into `01-...md` §2–§8, and it isn't in the later IT alignment notes captured in `01-...md` §9. It was given with an exact structure to implement verbatim, which is what's modeled here — this entity is the reason this document introduced the 🟣 tag (§2). A few notes on the structure as given:
...
Because this entity carries no prior scope-document trace, **`01-PROJECT_VISION_AND_PHASE1_SCOPE.md` has been updated with a matching addendum** recording Foundation Leaders as newly in-scope — see `01-...md` §10 — so the scope-of-record and this schema don't silently drift apart the way `AboutMap` briefly did in the opposite direction.
...
| `Leader` + Translation | 🟣 New requirement, direct instruction | No mock or scope-doc precedent at all — exact structure given; see §4.2. `01-...md` §10 records it as newly in-scope. |
