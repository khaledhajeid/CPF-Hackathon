...
- **`source` (`seed`/`impact_system`)** — 🔵 scope-derived directly from `01-PROJECT_VISION_AND_PHASE1_SCOPE.md` §7's Impact System integration strategy: Phase 1 activities are seeded data, Phase 2 activities may be synced from the external Impact System, and the field exists so that transition is a data-flag change, not a schema change.
...
**`NewsCategory` + Translation** — 🟢 Every category value (`أخبار المؤسسة`, `إنجازات الشباب`, `شراكاتنا`, `أخبار الفرص`) is observed directly across `newsList[]` and `heroSliderNews[]` in `newsData.js`.
...
- **One `NewsArticle` table instead of a separate hero-carousel model** — 🟡 architectural. `heroSliderNews[]` and `newsList[]` are two different mock arrays, but their items are structurally the same "thing" (a news item with media, a title, a category, a date) with `heroSliderNews` just being a flagged subset. `is_hero_carousel` and `is_featured` booleans capture that without duplicating the whole model.
...
| `Activity` + Translation | 🟢 Mostly mock-derived, 🟡 `program_id`, 🔵 `source` | The entity that prompted this whole document — see §4.2. |
| `NewsCategory` + Translation | 🟢 Mock-derived | Category strings observed across two mock arrays. |
| `NewsArticle` + Translation | 🟢 Mostly mock-derived, 🔵 `body` | Unifies two mock arrays into one model; full body is anticipatory. |
| `FieldLensImage` + Translation | 🟢 Mock-derived | `pulseImages[]`. |
| `SuccessStory` + Translation | 🟢 Mostly mock-derived, 🟡 `governorate_id` normalization | `allStories[]`, location normalized against the shared lookup. |
