...
import { Calendar, MapPin, Award, ArrowUpLeft, Briefcase, Target, Users, LayoutGrid, Timer, Filter, Search, ArrowLeft } from 'lucide-react';
import EventDetailsDrawer from '../events/EventDetailsDrawer';
import JordanMap from './JordanMap';
import { allEvents } from '../../data';

// 🟢 ضفنا onNavigate هنا عشان نقدر ننتقل لصفحة الفعاليات الكاملة
export default function HomeEvents({ activeFilters, setActiveFilters, handleRegisterClick, onNavigate }) {
  ...
  // 🟢 حددنا أول 6 فعاليات فقط للعرض في الصفحة الرئيسية
  ...
  return (
    <div id="events-section" className="py-24 bg-[#F8FAFC] relative font-sans" dir="rtl">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        
        <div className="mb-10 text-right">
          <div className="flex items-center gap-3">
            <img src="/arrow-yellow.svg" className="w-8 h-8 shrink-0" alt="" />
            <h2 className="text-3xl md:text-5xl font-black text-[#1a1c1d] tracking-tight">
              {isPersonalized ? "فرص مصممة خصيصاً لك" : "اكتشف أحدث الفرص المتاحة"}
            </h2>
          </div>
        </div>

        <div className="bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-gray-100 mb-10 flex flex-col xl:flex-row items-center gap-4 xl:gap-5 justify-start">
            ...
            {pathwaysData.map(path => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 whitespace-nowrap px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300 border-2 ${
          ...
          <div className="flex items-center gap-3 w-full xl:w-auto overflow-x-auto pb-2 xl:pb-0 scrollbar-hide shrink-0">
            <span className="text-sm font-bold text-gray-400 whitespace-nowrap flex items-center gap-2">
          ...
          <div className="w-full xl:w-80 shrink-0 relative">
            ...
            <input
              ...
              className="w-full bg-gray-50/80 border border-gray-100 text-gray-900 font-bold text-sm py-3.5 pr-11 pl-4 rounded-xl hover:bg-gray-100 focus:bg-white focus:border-[#721F31]/30 focus:ring-4 focus:ring-[#721F31]/5 outline-none transition-all"
            />
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-start">
          ...
          <div className="w-full lg:w-[35%] xl:w-[32%] shrink-0 lg:sticky lg:top-24">
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 relative flex flex-col min-h-[640px]">
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4 shrink-0">
                  <div>
                    <h3 className="font-black text-gray-900 text-xl">اختر المحافظة</h3>
                    <p className="text-xs font-bold text-gray-500 mt-1">اضغط على الخريطة لتصفية الفرص</p>
                  </div>
                  {activeFilters.location !== 'الكل' && (
                    <button 
                      onClick={() => setActiveFilters(prev => ({ ...prev, location: 'الكل' }))}
                      className="text-[10px] font-bold bg-gray-100 text-gray-600 px-3 py-1.5 rounded-full hover:bg-gray-200 transition-colors shrink-0 cursor-pointer"
                    >
                      إلغاء التحديد
                    </button>
                  )}
                ...
                <div className="flex-grow w-full flex items-center justify-center pt-4 pb-4">
                  <JordanMap 
          ...
          <div className="w-full lg:w-[65%] xl:w-[68%]">
            ...
            {isPersonalized && (
              <div className="mb-6 flex flex-wrap items-center gap-2 text-sm font-bold text-gray-500 bg-white px-4 py-3 rounded-xl border border-gray-100 shadow-sm inline-flex">
                <span>نعرض الفرص لـ:</span>
                <span className="text-[#721F31]">{activeFilters.pathway}</span>
                ...
                {searchQuery !== '' && <><span className="text-gray-300">•</span><span className="text-[#721F31]">بحث: "{searchQuery}"</span></>}
                <span className="text-gray-400 font-medium mr-2">({filteredEvents.length} نتيجة)</span>
              </div>
            )}
            ...
            <AnimatePresence mode="wait">
              {displayedEvents.length > 0 ? (
                <div className="flex flex-col gap-10">
                  ...
                  >
                    {/* 🟢 نقوم بعمل الخريطة (Map) على displayedEvents (الـ 6 الأولى) فقط */}
                      ...
                      return (
                        ...
                        >
                          <div className="relative h-48 overflow-hidden bg-gray-100 shrink-0">
                            ...
                            {isUrgent && (
                              <div className="absolute top-3 left-3 bg-red-600/90 backdrop-blur-sm text-white px-3 py-1.5 rounded-md text-[10px] font-black flex items-center gap-1.5 shadow-lg">
                                <Timer className="w-3 h-3" /> ينتهي قريباً
                              </div>
                            )}

                            <div className={`absolute top-3 right-3 px-3 py-1.5 text-[10px] font-black rounded-md shadow-md ${tagStyle.bg} ${tagStyle.text}`}>
                              {event.pathway}
                            </div>
                          </div>
                          ...
                          <div className="p-6 flex flex-col flex-grow">
                            <div className="mb-6">
                              <h3 className="font-black text-base text-gray-900 mb-4 line-clamp-2 group-hover:text-[#721F31] transition-colors leading-relaxed">
                                {event.title}
                              </h3>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-2 text-gray-500 text-xs font-bold">
                                  <MapPin className="w-4 h-4 text-gray-400" /> <span className="truncate">{event.city} - {event.location}</span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                              <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-[#C08F2D] font-black text-xs">+{event.points} نقطة</span>
                                </div>
                              </div>

                              <button
                                onClick={(e) => { e.stopPropagation(); setSelectedEvent(event); }}
                                className="flex items-center gap-2 bg-[#721F31] hover:bg-[#5a1826] text-white px-5 py-2.5 rounded-xl font-black text-xs transition-colors shadow-md group/btn"
                              >
                                سجل الآن
                                <ArrowUpLeft className="w-4 h-4 transform group-hover/btn:-translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                              </button>
                            </div>
                          </div>
                        </motion.div>
                  ...
                  {/* 🟢 زر التصفح الإضافي: يظهر فقط إذا كان هناك فعاليات أكثر من 6 */}
                  {filteredEvents.length > 6 && (
                    <div className="flex justify-center mt-4">
                      <button
                        onClick={() => onNavigate('programs')}
                        className="flex items-center gap-3 bg-white border-2 border-[#721F31] text-[#721F31] hover:bg-[#721F31] hover:text-white px-8 py-4 rounded-2xl font-black text-sm transition-all duration-300 shadow-sm hover:shadow-lg group"
                      >
                ...
                <motion.div 
                  ...
                  className="py-20 flex flex-col items-center justify-center text-center bg-white rounded-3xl border border-dashed border-gray-200 h-full"
                >
                  <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-6">
                    <Search className="w-10 h-10 text-gray-300" />
                  </div>
                  <p className="text-gray-900 font-black text-2xl mb-2">لا توجد نتائج مطابقة</p>
                  <p className="text-gray-500 font-medium max-w-md mb-8">لم نتمكن من إيجاد فرص تطابق الفلاتر أو البحث الحالي. جرب كلمات مختلفة أو أعد ضبط الفلاتر.</p>
                  <button 
                    onClick={() => { setActiveFilters({ pathway: 'الكل', location: 'الكل' }); setAgeFilter('الكل'); setSearchQuery(''); }} 
                    className="bg-[#C08F2D] hover:bg-[#a67b25] text-white px-8 py-3.5 rounded-xl font-black text-sm transition-colors shadow-lg cursor-pointer"
                  >
