...
export default function HomeEvents({ activeFilters, setActiveFilters, handleRegisterClick, onNavigate }) {
  ...
  const [searchQuery, setSearchQuery] = useState(''); // 🟢 إعادة الـ State الخاص بالبحث
  
  const [selectedEvent, setSelectedEvent] = useState(null);
  
  const ageRanges = ['الكل', '13 - 17', '18 - 23', '24 - 30'];

  // 🟢 المسارات الثلاثة الرسمية
  const pathwaysData = [
  ...
  // 🟢 ألوان المسارات الثلاثة
  const getTagStyle = (pathway) => {
    switch(pathway) {
      case 'تعلّم': return { bg: 'bg-[#2b307e]', text: 'text-white' };
      case 'قُد': return { bg: 'bg-[#8a1538]', text: 'text-white' };
  ...
  return (
    <div id="events-section" className="py-16 md:py-24 bg-[#F4F7FA] relative font-sans" dir="rtl">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 🟢 شريط الفلاتر مع البحث (تم استخدام flex-wrap و flex-col للموبايل لمنع الـ Overflow) */}
        <div className="bg-white p-4 rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 mb-8 md:mb-10 flex flex-col xl:flex-row items-start xl:items-center gap-4 xl:gap-5 justify-start w-full">
              ...
              return (
                <button
                  ...
                  className={`flex flex-col md:flex-row items-center justify-center md:justify-start gap-1.5 md:gap-2 px-2 md:px-6 py-3 rounded-xl font-bold text-[11px] md:text-sm transition-all duration-300 border-2 w-full md:w-auto text-center ${
                    isActive ? 'bg-[#8a1538] border-[#8a1538] text-white shadow-md' : 'bg-gray-50 border-transparent text-gray-500 hover:border-gray-200 hover:bg-gray-100'
                  }`}
                >
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3 w-full xl:w-auto">
             ...
             <div className="md:hidden relative w-full group">
               <select 
                 className="w-full bg-gray-50 border border-gray-100 text-gray-700 font-bold text-sm py-3 px-4 rounded-xl appearance-none outline-none focus:border-[#8a1538] focus:ring-1 focus:ring-[#8a1538]/20"
                 value={ageFilter}
               ...
               {ageRanges.map(age => (
                 <button
                   ...
                   className={`whitespace-nowrap px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 border-2 ${
                     ageFilter === age 
                       ? 'bg-[#8a1538] border-[#8a1538] text-white shadow-md' 
                       : 'bg-transparent border-[#8a1538] text-gray-700 hover:bg-[#8a1538]/5'
                   }`}
                 >
                   {age}
                 </button>
          ...
          <div className="w-full xl:w-80 shrink-0 relative mt-2 xl:mt-0">
            ...
            <input
              ...
              className="w-full bg-gray-50/80 border border-gray-100 text-gray-900 font-bold text-[13px] md:text-sm py-3.5 pr-11 pl-4 rounded-xl hover:bg-gray-100 focus:bg-white focus:border-[#8a1538]/30 focus:ring-4 focus:ring-[#8a1538]/5 outline-none transition-all"
            />
          </div>
                ...
                <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <pattern id="grid-pattern" width="40" height="40" patternUnits="userSpaceOnUse">
                      <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#8a1538" strokeWidth="1"/>
                    </pattern>
                  </defs>
          ...
          <div className="w-full lg:w-[65%] xl:w-[68%]">
            ...
            {isPersonalized && (
              <div className="mb-6 flex flex-wrap items-center gap-2 text-[12px] sm:text-sm font-bold text-gray-500 bg-white px-4 py-3 rounded-xl border border-gray-100 shadow-sm inline-flex w-full sm:w-auto">
                ...
                <span className="text-[#8a1538] truncate max-w-[150px] sm:max-w-none">{activeFilters.pathway}</span>
                ...
                {searchQuery !== '' && <><span className="text-gray-300">•</span><span className="text-[#8a1538] truncate max-w-[100px]">"{searchQuery}"</span></>}
              ...
              {displayedEvents.length > 0 ? (
                        ...
                        >
                          <div className="relative h-44 md:h-48 overflow-hidden bg-gray-100 shrink-0">
                            <img
                              ...
                              onError={(e) => { e.target.onerror = null; e.target.src = 'https://via.placeholder.com/600x400/8a1538/FFFFFF?text=Crown+Prince+Foundation'; }}
                          ...
                          <div className="p-5 md:p-6 flex flex-col flex-grow">
                            <div className="mb-5 md:mb-6">
                              <h3 className="font-black text-[15px] md:text-base text-gray-900 mb-3 md:mb-4 line-clamp-2 group-hover:text-[#8a1538] transition-colors leading-relaxed">
                                {event.title}
                              </h3>
                            ...
                            <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                              ...
                              <button
                                ...
                                className="flex items-center gap-2 bg-[#8a1538] hover:bg-[#680f2a] text-white px-4 md:px-5 py-2 md:py-2.5 rounded-xl font-black text-xs transition-colors shadow-md group/btn cursor-pointer"
                              >
                                سجل الآن
                ...
                >
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-gray-50 rounded-full flex items-center justify-center mb-5 md:mb-6">
                    <SearchX className="w-8 h-8 md:w-10 md:h-10 text-gray-300" />
                  </div>
