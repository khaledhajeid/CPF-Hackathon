...
// 🟢 مكون القائمة المنسدلة المخصص (تم حل مشكلة الاختفاء)
const CustomDropdown = ({ options, value, onChange, placeholder }) => {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef(null);

  // 🟢 هذا الكود بيخلي القائمة تسكر بس لما تكبس برا (Click Outside)
  useEffect(() => {
  ...
  return (
    // 🟢 شلنا الـ onMouseLeave وحطينا الـ ref هون
    <div className="relative w-full sm:w-auto flex-grow" ref={dropdownRef}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between bg-white border border-gray-200 text-gray-700 font-bold text-sm py-3.5 px-4 rounded-xl hover:border-gray-300 focus:border-[#721F31] focus:ring-2 focus:ring-[#721F31]/20 outline-none transition-all cursor-pointer"
      >
        <span>{value === 'الكل' ? placeholder : value}</span>
            ...
            {options.map((option) => (
              <div
                ...
                className={`px-4 py-3 text-sm font-bold cursor-pointer transition-colors flex items-center justify-between ${
                  value === option ? 'bg-[#721F31] text-white' : 'hover:bg-[#5a5a5b] text-gray-100'
                }`}
              >
                {option}
...
export default function EventsExplorer({ handleRegisterClick }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
      ...
      <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 mb-8 flex flex-col xl:flex-row items-center gap-3 w-full">
        ...
        <div className="w-full xl:w-auto flex flex-wrap sm:flex-nowrap gap-3 relative z-40">
          ...
          <div className="xl:w-36 flex-grow relative z-30">
        ...
        <div className="w-full lg:w-[35%] xl:w-[36%] shrink-0 lg:sticky lg:top-24 z-10">
          <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 relative flex flex-col min-h-[640px]">
            <div className="absolute inset-0 z-0 opacity-[0.03] pointer-events-none rounded-3xl overflow-hidden">
              <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg"><defs><pattern id="grid-pattern-2" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="#721F31" strokeWidth="1"/></pattern></defs><rect width="100%" height="100%" fill="url(#grid-pattern-2)" /></svg>
            </div>
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4 shrink-0">
                <div>
                  <h3 className="font-black text-gray-900 text-xl">الموقع الجغرافي</h3>
                  <p className="text-xs font-bold text-gray-500 mt-1">اختر المحافظة لتصفية الفعاليات</p>
                </div>
                {eventLocation !== 'الكل' && (
                  <button onClick={() => setEventLocation('الكل')} className="text-[10px] font-bold bg-gray-100 text-gray-600 px-3 py-1.5 rounded-full hover:bg-gray-200 transition-colors shrink-0 cursor-pointer">إلغاء</button>
                )}
              </div>
        ...
        <div className="w-full lg:w-[65%] xl:w-[68%] z-10">
          <AnimatePresence mode="wait">
            {filteredEvents.length > 0 ? (
              <motion.div key="events-grid" variants={staggerContainer} initial="hidden" animate="visible" exit="hidden" className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-48 overflow-hidden bg-gray-100 shrink-0">
                        ...
                        {isUrgent && (
                          <div className="absolute top-3 left-3 bg-red-600/90 backdrop-blur-sm text-white px-3 py-1.5 rounded-md text-[10px] font-black flex items-center gap-1.5 shadow-lg"><Timer className="w-3 h-3" /> متاح الآن</div>
                        )}
                        <div className={`absolute top-3 right-3 px-3 py-1.5 text-[10px] font-black rounded-md shadow-md bg-white text-[#721F31]`}>{event.pathway}</div>
                      </div>
                      <div className="p-6 flex flex-col flex-grow">
                        <div className="mb-6">
                          <h3 className="font-black text-base text-gray-900 mb-4 line-clamp-2 group-hover:text-[#721F31] transition-colors leading-relaxed">{event.title}</h3>
                          <div className="space-y-2">
                            <div className="flex items-center gap-2 text-gray-500 text-xs font-bold"><Calendar className="w-4 h-4 text-gray-400" /> {event.date}</div>
                            <div className="flex items-center gap-2 text-gray-500 text-xs font-bold"><MapPin className="w-4 h-4 text-gray-400" /> <span className="truncate">{event.city} - {event.location}</span></div>
                          </div>
                        </div>
                        <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                          <div className="flex flex-col">
                            <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">المكافأة</span>
                            <div className="flex items-center gap-1"><Award className="w-4 h-4 text-[#C08F2D]" /><span className="text-[#C08F2D] font-black text-xs">+{event.points} نقطة</span></div>
                          </div>
                          <button 
                          ...
                          >
                            التفاصيل <ArrowUpLeft className="w-4 h-4 transform group-hover/btn:-translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                          </button>
                        </div>
                      </div>
                    </motion.div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-20 flex flex-col items-center justify-center text-center bg-white rounded-3xl border border-dashed border-gray-200 h-full">
                <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-6"><Search className="w-10 h-10 text-gray-300" /></div>
                <p className="text-gray-900 font-black text-2xl mb-2">لا توجد نتائج مطابقة</p>
                <p className="text-gray-500 font-medium max-w-md mb-8">لم نتمكن من إيجاد فرص تطابق الفلاتر المحددة. جرب تغيير الإعدادات.</p>
                <button onClick={() => { setEventTypeFilter('الكل'); setEventPathwayFilter('الكل'); setEventAgeFilter('الكل'); setEventLocation('الكل'); setEventSearch(''); }} className="bg-[#C08F2D] hover:bg-[#a67b25] text-white px-8 py-3.5 rounded-xl font-black text-sm transition-colors shadow-lg cursor-pointer">إعادة ضبط جميع الفلاتر</button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
