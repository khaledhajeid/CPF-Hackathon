...
export default function EventsExplorer({ handleRegisterClick }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
      ...
      {/* 🟢 شريط الفلاتر: تم ترتيبه عمودياً (Stacked) ع الموبايل عشان ما يزيح يمين وشمال */}
      <div className="bg-white p-4 rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 mb-8 flex flex-col xl:flex-row items-start xl:items-center gap-4 w-full">
        ...
        {/* فلاتر القوائم المنسدلة: تاخذ العرض كامل عالموبايل ومرتبة فوق بعض */}
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-44 md:h-48 overflow-hidden bg-gray-100 shrink-0">
                        ...
                        <div className={`absolute top-3 right-3 px-2.5 py-1.5 text-[10px] font-black rounded-md shadow-md bg-white text-[#721F31]`}>{event.pathway}</div>
                      </div>
                      <div className="p-5 flex flex-col flex-grow">
                        <div className="mb-5">
                          <h3 className="font-black text-[15px] text-gray-900 mb-3 line-clamp-2 group-hover:text-[#721F31] transition-colors leading-relaxed">{event.title}</h3>
                          <div className="space-y-2">
                            <div className="flex items-center gap-2 text-gray-500 text-xs font-bold"><Calendar className="w-3.5 h-3.5 text-gray-400" /> {event.date}</div>
                            <div className="flex items-center gap-2 text-gray-500 text-xs font-bold"><MapPin className="w-3.5 h-3.5 text-gray-400" /> <span className="truncate">{event.city} - {event.location}</span></div>
                          </div>
                        </div>
                        <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                          ...
                          <button 
                            ...
                            className="flex items-center gap-2 bg-[#721F31] hover:bg-[#5a1826] text-white px-4 md:px-5 py-2.5 rounded-xl font-black text-[11px] md:text-xs transition-colors shadow-md group/btn"
                          >
