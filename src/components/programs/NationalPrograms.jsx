...
export default function NationalPrograms({ onNavigate, setActiveProgramName }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
      ...
      {/* 🟢 شريط الفلاتر: تم تحويله لـ wrap عشان يلف الكلمات براحته ع الموبايل بدون سحب مزعج */}
      <div className="flex flex-wrap gap-2 w-full mb-8 md:mb-10 justify-center">
        {categories.map(category => {
          const isActive = activeProgramFilter === category;
          return (
            <button
              key={category} onClick={() => setActiveProgramFilter(category)}
              className={`px-4 md:px-5 py-2 md:py-2.5 rounded-xl font-bold text-[13px] md:text-sm transition-all duration-300 whitespace-nowrap border-2 cursor-pointer ${isActive ? 'bg-[#721F31]/5 border-[#721F31] text-[#721F31] shadow-sm' : 'bg-white border-transparent text-gray-500 hover:border-gray-200 hover:bg-gray-50'}`}
            >
              {category}
            </button>
          );
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {filteredPrograms.map((program) => {
          ...
          return (
            ...
            >
              <div className="relative h-48 md:h-56 overflow-hidden bg-gray-50 shrink-0">
                ...
                <div className="absolute top-4 right-4 md:top-5 md:right-5">
                  <span className={`flex items-center gap-1.5 border px-2.5 md:px-3 py-1 md:py-1.5 rounded-lg text-[10px] font-black backdrop-blur-md shadow-sm bg-white/90 ${tagStyle.split(' ')[1]}`}>
                    {program.pathway}
                  </span>
                </div>
                <div className={`absolute bottom-4 left-4 md:bottom-5 md:left-5 w-10 h-10 md:w-12 md:h-12 rounded-xl bg-gradient-to-br ${program.color} flex items-center justify-center shadow-xl border-2 border-white/20 backdrop-blur-sm transform group-hover:-translate-y-2 transition-transform duration-500`}>
                  <PathwayIcon className="w-4 h-4 md:w-5 md:h-5 text-white" />
                </div>
              </div>
              <div className="p-5 md:p-6 flex flex-col flex-grow bg-white">
                <h3 className="font-black text-lg md:text-xl text-gray-900 mb-2 md:mb-3 group-hover:text-[#721F31] transition-colors leading-tight">{program.title}</h3>
                <p className="text-gray-500 text-[13px] md:text-sm font-medium leading-relaxed mb-5 md:mb-6 flex-grow">{program.description}</p>
                <div className="pt-4 md:pt-5 border-t border-gray-50 mt-auto flex items-center justify-between w-full text-gray-800">
                  <span className="font-black text-[13px] md:text-sm group-hover:text-[#721F31] transition-colors">استكشف البرنامج</span>
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-gray-50 flex items-center justify-center group-hover:bg-[#721F31] transition-colors border border-gray-100 group-hover:border-[#721F31]">
                    <ArrowUpLeft className="w-3.5 h-3.5 md:w-4 md:h-4 text-gray-400 group-hover:text-[#C08F2D] transform group-hover:-translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                  </div>
                </div>
              </div>
            </motion.div>
